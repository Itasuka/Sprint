...
fonction  afficherIndex (){
    $ contenuCat = '<body>
            <form method="POST" action="site.php">
                <jeu de champs>
                    <legend> saisir votre login et vos mots passés </legend>
                    <p>
                        <label name="log">Votre login :</label>
                        <input type="text" name="login" id="login" placeholder="saisir votre login" requis />
                    </p>
                    <p>
                        <label name="mdp">Votre mot de passe :</label>
                        <input type="password" name="mdp" id="mdp" placeholder="saisir votre mot de passe" requis />
                    </p>
                    <p>
...
fonction  afficherAcceuilDirecteur (){
...
fonction  afficherAcceuilAgent (){
	$ contenuCat = '<form id="Agent" action="site.php" method="POST">
                        ...
                        <p>
                                ...
                                <input type="submit" value="dépôt ou retrait sur le compte d\'un client" name="DouRCompteCli" />
                ...
                <form id="ModifTnfoCli" action="site.php" method="POST">
                        <jeu de champs>
                                <p><legend>donner le nom du client concerné :</legend><input type="text" name="NomCli">
                                </p>
                        </fieldset>
                        <jeu de champs>
                                <p><legend>adresse :</legend><input type="text" name="adresseCli"></p>
                                <p><legend>numéro de téléphone</legend><input type="text" name="NumTelCli"></p>
...
fonction  afficherAcceuilConseille (){
	$ contenuCat = '<form id="debutConseille" action="site.php" method="POST">
            ...
            <form id="ajoutCli"action="site.php" method="POST">
                <jeu de champs>
                    ...
                    <p><label>numéro de téléphone du client :</label><input type="text" name="NumTelCli"></p>
            ...
            <form id="modifDécouvert"action="site.php" method="POST">
                <jeu de champs>
                    <p><label>Id du compte :</label><input type="text" name="IDCompte"></p>
                    <p><label>Id du chient :</label><input type="text" name="Idcli"></p>
                    <p><label>nom du client :</label><input type="text" name="NomCli"></p>
                    <p><label>Nouveau montant à découvert</label><input type="text" name="NewDecouvert"></p>
...
fonction  afficherStatsClients ( $ tab ){
    foreach ( $ tab  comme  $ ligne ){
...
fonction  afficherStatsSomme ( $ tab ){
    foreach ( $ tab  comme  $ ligne ){
...
fonction  afficheridentifiantClient ( $ tab ){
    foreach ( $ tab  comme  $ ligne ){
    $ contenu = "<p> Le client à l'identifiant suivant : " . $ ligne -> idlci . ".</p>" ;
    }
}

fonction  afficherPlanning1jour1employe ( $ tab ){
    $ h = 8 ;
    ...
    foreach ( $ tab [ 0 ] as  $ planning ){
        ...
        if (( $ planification -> heure )- 8 == $ h ){
            $ listedespieces = $ planning -> nompieces ;
            $ motif = $ planification -> nommotiv ;
            $ categorie = $ planification -> categorierdv ;
            $ idclient = $ planification -> idcli ;
            if (! isset ( $ idclient )){
                if ( $ categorie == "tache" ){
...
//onglet des comptes
    // en fonction de l'id 
fonction  afficherLesComptes ( onglet $ ){
    ...
    foreach ( $ tab  as  $ compte ) {
        $ contenu .= "<p><input type='radio' name='compteChoisi' onfocus='afficheDecouvertEtSolde(" . $ compte -> montantdecouvert . "," . $ compte -> solde . ")'/><label > Compte numéro " . $ compte -> idcompte . " de type " . $ compte -> nomCompte . " ouvrir le " . $ compte -> dateouverture . ". </label></p>"
    }
    $ contenu .= "<input type='submit' value= " ;
}

fonction  afficherErreurdeco ( $ erreur ){
    $ contenuCat = "" ;
    $ contenu = '<p>' . $ erreur . '</p>
...
fonction  afficherErreurco ( $ erreur ){
    $ contenuCat = "" ;
    $ contenu = '<p>' . $ erreur . '</p>
    <p><a href=site.php name="retour">Retour au site</a>' ;
    require_once ( 'gabarit.php' );
...
/*
faire les vues de l'accueil en fonction de la catégorie du client
